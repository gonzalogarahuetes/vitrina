...
impl Header {
    fn parse(bytes: &[u8]) -> Result<Header, HeaderError> {
        ...
        if  bytes.len() < 64 {
            return Err(HeaderError::TooShort{ expected: 64, got: bytes.len() })
        }
        ...
        let chunk_size = u32::from_le_bytes(bytes[24..28].try_into().expect("length checked above"));
        ...
        if padding != [0u8; 12] { 
            ...
            return Err(HeaderError::PaddingNotZero{ offset: total_index })
        }
...
mod tests {           // a submodule literally named `tests`
    ...
    fn rejects_short_input() {
        assert!(matches!(Header::parse(&VALID[0..40]), Err(HeaderError::TooShort { .. })));
    }
    ...
    fn rejects_wrong_version() {
        let mut bad = VALID;
        bad[4] = 9;
        assert!(matches!(Header::parse(&bad), Err(HeaderError::WrongVersion(9))));
    } 
    ...
    fn rejects_wrong_cipher() {
        let mut bad = VALID;
        bad[5] = 9;
        assert!(matches!(Header::parse(&bad), Err(HeaderError::WrongCipher(9))));
    } 
    ...
    fn rejects_reserved_non_zero() {
        let mut bad = VALID;
        bad[6] = 1;
        assert!(matches!(Header::parse(&bad), Err(HeaderError::ReservedNotZero{ offset: 6 })));
    } 
    ...
    fn rejects_padding_non_zero() {
        let mut bad = VALID;
        bad[63] = 1;
        assert!(matches!(Header::parse(&bad), Err(HeaderError::PaddingNotZero{ offset: 63 })));
    }
}

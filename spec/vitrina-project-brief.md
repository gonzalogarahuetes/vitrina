...
4. **One cryptographic implementation, not one per platform.** `crates/envelope` is the only place envelope cryptography exists: compiled to WASM for the browser, linked by FFI from Swift and Kotlin. Its primitives come from **RustCrypto** — pure Rust, because a C dependency in a WASM build costs an Emscripten toolchain to buy a guarantee one external test vector buys instead (encryption spec §9.2). **Not Web Crypto**: browser-only and no Argon2id. **And not a per-platform primitive library either**, because that reintroduces the divergence the crate exists to prevent. _This clause read "libsodium everywhere" until 21 August 2026, which described each client linking libsodium directly. That was never what was built — the component table said "one implementation, three platforms" from the first session — and the two never agreed. libsodium now appears nowhere in Vitrina; it survives only as the source of external test vectors._
...
| Hosting                 | **Decided 20 August 2026: EU, and Hetzner.** VPS in Falkenstein or Helsinki with Hetzner Object Storage alongside it — one German entity, one bill. **Acceptance condition: run B.4's test against the real provider before committing** — write an object, fetch a byte range through a presigned URL, assert the bytes. **Add a third assertion: read-after-write consistency for new objects.** The upload flow issues a confirming `HEAD` immediately after `PutObject` and treats a miss as _not yet visible_ rather than as failure; whether that branch is reachable on SeaweedFS or Hetzner is unknown, and a store that is _not_ read-after-write consistent makes it the common path rather than the rare one. AWS S3 has been consistent for new objects since 2020; neither of these is AWS. Hetzner's S3 compatibility is described as limited relative to AWS, which is exactly what that test catches. **Why same-provider matters:** §10.1 proxying means every byte travels bucket → server → client, so co-locating makes the first hop internal and turns doubled egress back into a single charge. Verify the internal-traffic terms explicitly. **What EU hosting actually buys:** the relay holds ciphertext, so a US legal request would yield nothing readable — what it would yield is metadata (addresses, album structure, access logs, timing). EU providers protect the metadata; the architecture already protects the content. **No CDN is needed** — `no-store` makes nothing cacheable and proxying puts the API in the path regardless, so the usual Cloudflare role does not apply. Alternative if managed Postgres is wanted: Scaleway, at €0.01/GB egress. Considered and not chosen: Cloudflare R2 (zero egress, but US-headquartered). |
| Owner account model     | **Decided 20 August 2026.** Email and password. Email is a memorable username, not a recovery channel — the key decision lives in §11. **No key recovery in v1**, structured so it can be added without a migration. The owner password's Argon2id parameters remain open and are a single **client-side** set sized for the weakest phone — the relay applies a peppered fast hash rather than a KDF (encryption spec §6.6), so no server-side figure exists. **Closed 14 September 2026: 64 MiB, t=3, p=1**, confirmed by phase-0-plan §8's V.1 at 2239 ms worst on a Helio G35 against a pre-registered 3000 ms limit. Encryption spec §6.2 carries them as normative; api-sketch §8.1 should drop its "provisional pending V.1" wording.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Owner password floor    | Client-side, since the relay never sees a password and cannot enforce one (encryption spec §6.6.2). Newly consequential rather than cosmetic: §6.6.2 records that `wrapped_master` is unpeppered, so a weak owner password is offline-attackable at rest and Argon2id at §6.2's parameters is the whole defence. PR 2b's signup form owes a number                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Owner password policy   | **decided 15 September 2026, client-side.** Minimum 12 characters, no composition rules, maximum 256 to bound the copy into WASM, and a bundled top-100k blocklist — a floor alone admits `passwordpassword`, and the blocklist is what does the work at the low end. Against Argon2id at 64 MiB / t=3, 8 characters of ordinary human choice falls in hours to days and 12 buys months to years. **Enforced at signup only, never at login:** a client that re-checks the floor at login locks out every account created before it rose, which is encryption spec §6.6.2's rejecting-client failure in a new place. The relay cannot enforce any of this — it never sees a password — so this is the client's alone, and §6.6.2 records why it is load-bearing rather than cosmetic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

...
**No column ever holds a plaintext filename**, and none holds key material. `recipients.wrapped` holds a wrapped blob for passphrase recipients; QR recipients store nothing at all (encryption spec §6.1). `owners.auth_hash` and `owner_keys.wrapped_master` are the same class — ciphertext and hashes, never a key (§9.3).

_The column was named `recipients.wrapped_key` here until 21 August 2026; it is `wrapped`, as schema §3 defines it and as §9.3's hash list already spelled it. So the brief disagreed with itself one subsection apart, and a wrong column name in the document that wins conflicts is worse than none: it reads as authoritative and resolves to nothing._
...
**`owners` stays minimal in the Phase 0 migration, and the reason is no longer that §12 is open.** §12 decided the account model on 20 August 2026 — **email and password** — so this paragraph's original reasoning ("a migration is not the place to resolve an open decision") has expired. What remains true is the outcome: `001_initial_schema.sql` has already applied with only `id` and `created_at`, and recording a decision made afterwards does not retrospectively change what shipped. The columns the model needs — `email`, `auth_hash`, and the relay's four KDF columns — land in a **Phase 1 migration**, enumerated in schema §3.

_This paragraph said "§12 still lists the owner account model as undecided — email required, or invite-only" until 21 August 2026. §12 had said "decided" for a day, so the brief contradicted itself, and it is the document that wins conflicts._
...
**Four different kinds of hash, and no two are interchangeable.**

1. **`token_hash` columns** — SHA-256 of a 32-byte random token. There is nothing to brute-force in 256 bits of entropy, and a password hash here would be pure per-request cost.
2. **`recipients.wrapped`** — protected by Argon2id, because a human-transcribable passphrase _is_ brute-forceable (§6.3). Parameters per recipient row.
3. **The client's Argon2id over the owner's password** — the only one of the four that is never stored. It produces the KEK that unwraps `K_master` and the proof the client sends; only its _parameters_ are stored, on `owner_keys`, bounded by a mobile WASM heap (encryption spec §6.2) and returned by `/login/params`.
4. **`owners.auth_hash`** — the relay's own Argon2id, applied to the **proof** it receives and never to a password, which it never sees. Its parameters live on `owners`, chosen for a server under concurrency, and are never returned to a client.

_It was three until 21 August 2026, and the count drifted for the ordinary reason: (3) and (4) arrived together as "the owner password's Argon2id" and read as one thing. They are two layers, two purposes, two parameter sets, in two tables — the conflation this list exists to prevent, in the document that wins conflicts._

**(4) is the subtle case, and the distinction from (1) is what belongs in the migration comments**, because the two columns look identical: both hold 32 high-entropy bytes, and only one of them is _known_ to. The relay mints its own tokens, so their entropy is a fact. A proof's entropy is a claim about what a client did with Argon2id, and the relay cannot check it — so a fast hash there would be safe exactly as long as every client implementation stays correct. That is #17's test rather than a performance question: it works, **wrongly**, without the relay's own layer. Encryption spec §6.6.
...
**`owners` is deferred, not designed.** With only `id` and `created_at` there is nothing to authenticate against, so the Phase 1 owner flow cannot start without adding to it. That is acceptable in a provisional migration and should be labelled as such.

_Two corrections here, 21 August 2026._ This paragraph ended "a password column would not resolve §12 — both candidate account models need one", which was stale twice over. **§12 is decided** (email and password, 20 August). And **there is no password column, in either sense**: `owners.auth_hash` holds the relay's Argon2id over a received proof, and the password never reaches the relay at all (§9.3 (4), encryption spec §6.6). Calling it "a password column" is the shorthand that makes someone write a route accepting a password — which is exactly what non-negotiable #16 forbids.
...
| Owner account model     | **Decided 20 August 2026.** Email and password. Email is a memorable username, not a recovery channel — the key decision lives in §11. **No key recovery in v1**, structured so it can be added without a migration. **Two Argon2id parameter sets remain open, not one** — the client's, bounded by a mobile WASM heap, and the relay's, chosen for a _server under concurrency_ (§9.3, encryption spec §6.6). Neither may be inherited from the other or from §6.2's 64 MiB. _This cell said "the owner password's Argon2id parameters" singular, with the server-under-concurrency instruction attached to all of it, until 21 August 2026; that instruction is right for the relay's set and wrong for the client's._                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

...
**The pepper must be recoverable but stored apart from the database.** Losing it bricks every account; storing it in the same backup artifact collapses this to an unpeppered hash, since one snapshot theft yields both. Different artifact, different access control. Absent at boot is a hard error (#17) — note its failure mode differs from the decoy secret's: a missing pepper breaks every login loudly, _except_ on a fresh deployment with no accounts, where generate-if-missing works fine and the damage surfaces later.

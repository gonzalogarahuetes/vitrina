...
| §7.2 no token in a query string | Same walk: no schema declares a `querystring`, `params` or `headers` section at all. Scoped that way deliberately — `token` in `/login`'s *response* is the session the route exists to return, and an earlier version of the walk flagged it |
...
- **`/login/params` returns the chosen values on a miss** (§7.5), because a decoy
  ...
  _As of 21 September 2026 the constant lives in the composition root, not in `packages/shared`: that package exports only a `types` condition and has no build, so it cannot hold a runtime value. It moves when `shared` gains a `tsc` build — small and delegable. The placement above is right; the package is behind it._

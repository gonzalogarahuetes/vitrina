...
| §8.1 signup validates against floors, not chosen values                             | **PR 2b, prose only.** A test posting `/signup` with parameters _above_ the v1 values and asserting `201`; another posting one below each floor and asserting `400`. The first is the one that matters — a schema pinned to the chosen values passes the second and fails the first, and nobody writes the first by instinct                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| §8.1 decoy parameters equal the constant real rows carry                            | **PR 2b, prose only.** Assert `/login/params` for an unknown address returns the values from the shared constant, and that a fresh `/signup` from the reference client writes the same three integers — the one comparison that catches the two drifting apart                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| §8.2 one normalisation function, called before every lookup                         | **PR 2b, prose only.** Sign up with one spelling, hit `/login/params` and `/login` with a differently-cased, differently-spaced spelling of the same address, assert the real salt comes back and the login succeeds. Then the negative: two spellings that normalise apart must yield two accounts                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| §8.2 the secret is required at boot                                                 | **PR 2b, prose only.** `buildServer` with the variable absent, and with it 31 bytes, must reject before listening — same shape as the CORS-origin boot test in §6.1. The generate-if-missing fallback is the specific implementation this test fails                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| §7.5 `/login` compares `auth_hash` in constant time, and runs the same path on miss | **PR 2b, prose only, and the timing half is §4.3's unassertable one.** What _is_ assertable: the compare is `timingSafeEqual` (or the equivalent) by import, and the miss path calls the same verification function as the hit path with the dummy row — a structural assertion, per §4.3's row above, not a stopwatch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| §8.3 `/owner/key` returns only the caller's row                                     | **PR 2b, prose only, and trivially true by construction** — no id in the path. Owed anyway: two owners, two sessions, each `GET` returns its own `wrapped_master` and not the other's. Cheap, and it is the test that fails if someone later adds an `?owner_id` "for admin"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
3. Apply the **unconditional** lowercase mapping to the whole string — the same
